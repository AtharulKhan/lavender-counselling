...
class Tp_Blocks_Ajax { 
                ...
                if(empty($userRegi)){				
                    ...
                    if(!empty($_POST['regiAction'])){
                        ...
                        if( in_array('sendemail',$action)){
                            ...
                            if(!empty($mail)){
                                /* translators: You have successfully registered to %s. We have emailed your password to the email address you entered. */
    ...
    public function tpgb_send_magic_link(){
        ...
        if(!empty($mlinkSend)){
            echo wp_json_encode( [ 'magicMsg'=> true , 'message'=> '<strong> '.sprintf(__( 'Magic Link Send...','tpgbp' )).'</strong>' ] );
        }else{
