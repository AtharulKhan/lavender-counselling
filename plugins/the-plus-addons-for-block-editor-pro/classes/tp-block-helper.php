...
class Tpgbp_Pro_Blocks_Helper {
	...
	public function __construct() {
        ...
        add_action('wp_ajax_nxt_form_pro_action',  array($this, 'nxt_form_pro_action_callback'));
        add_action('wp_ajax_nopriv_nxt_form_pro_action', array($this, 'nxt_form_pro_action_callback'));
	}
	...
	*/
    ...
    public function nxt_form_pro_action_callback() {
        ...
        if(empty($actionValues)){
            ...
            $response['data'] = 'No action values found';
        ...
        if (in_array('Email', $actionValues)) {
            $email_to = isset($first_decode['emailTo1']) && !empty($first_decode['emailTo1']) ? sanitize_email($first_decode['emailTo1']) : '';
            ...
            if (!empty($email_to) && !empty($subject)) {
                ...
                $emailHdg = isset($first_decode['emailHdg']) && !empty($first_decode['emailHdg']) ? sanitize_text_field($first_decode['emailHdg']) : 'You have received a new form submission:';
        ...
        if (in_array('Auto Respond Email', $actionValues)) {
            $email_to2 = isset($first_decode['emailTo2']) && !empty($first_decode['emailTo2']) ? sanitize_email($first_decode['emailTo2']) : get_bloginfo('admin_email');
            ...
            if (!empty($email_to2)) {
                ...
                $emailHdg2 = isset($first_decode['emailHdg2']) && !empty($first_decode['emailHdg2']) ? sanitize_text_field($first_decode['emailHdg2']) : 'You have received a new form submission:';
                $regular_fields = [];
                $excludedFields = [
                    'actionOption',
                    'nonce',
                ...
                $full_message2 = "<h2>$emailHdg2</h2>";
                    ...
                    foreach ($non_empty_fields as $key => $value) {
                        ...
                        $full_message2 .= "<p><strong>" . ucfirst($key) . ":</strong> $value</p>";
                    }
                ...
                if (isset($first_decode['metaDataOpt2']) && is_array($first_decode['metaDataOpt2'])) {
                    $full_message2 .= "<p><strong>Meta Data:</strong></p><ul>";
                    ...
                    foreach ($first_decode['metaDataOpt2'] as $metaData) {
                        ...
                        $full_message2 .= "<li><strong>$label:</strong> $value</li>";
                    }
        ...
        if (in_array('Database Entry', $actionValues)) {
            ...
            if ($insert_result === false) {
                $errors .= 'Failed to insert data into the database. Error: ' . $wpdb->last_error;
        ...
        if (in_array('WebHook', $actionValues)) {
            ...
            if (!filter_var($webhook_url, FILTER_VALIDATE_URL)) {
                $errors .= 'A valid URL was not provided. '; 
            } else {
                ...
                if (is_wp_error($webhook_response)) {
                    $response_message = 'Webhook request failed: ' . $webhook_response->get_error_message();
                    ...
                    $status_code === 200 
                        ? $response['data'] .= 'Webhook request successful. '
                        : $errors .= ($response_message = 'Webhook request failed with status code: ' . $status_code . '. Response: ' . $body);
        ...
        if (in_array('Slack', $actionValues)) {		
            ...
            if (empty($slack_token)) {
                $errors .='Slack token is required. ';
            }
        ...
        if (in_array('Discord', $actionValues)) {
            ...
            if (!filter_var($discord_webhook_url, FILTER_VALIDATE_URL)) {
                $errors .= 'A valid Discord webhook URL was not provided. ';
            } else {
                $data_to_send = array(
                ...
                if (is_wp_error($response_discord)) {
                    $response_message = 'Discord webhook request failed: ' . $response_discord->get_error_message();
                    $errors .= $response_message;
                } else {
                    ...
                    ? ($response['data'] .= 'Message sent successfully to Discord.') 
                    : ($errors .= $response_message = 'Failed to send message to Discord. Response: ' . $response_message);
        ...
        if (in_array('ActiveCampaign', $actionValues)) {
            ...
            if (is_wp_error($create_response)) {
                ...
                $errors .= 'Failed to connect to ActiveCampaign API. Error: ' . $error_message;
            } else {
                $actions_success['activecampaign'] = isset($create_response['contact']['id']) 
                ? ($response['data'] .= 'Contact added to ActiveCampaign successfully.') 
                : ($response['data'] .= 'Failed to add contact to ActiveCampaign. Check response details.');
            ...
            if (isset($first_decode['mApiKey'], $first_decode['mGrpId'])) {
                ...
                if (is_wp_error($result)) {
                    ...
                    if (isset($result['id'])) { 
                        ...
                        $response['data'] = 'Subscriber added successfully.';
                        $actions_success['mailerLite'] = true; 
                    } else {
                        $response['data'] = 'Failed to add subscriber. Check response details.';
                    }
                }
            } else {
                $errors .= 'API key, email, name, and group ID are required fields.'; 
            }
            ...
            if (isset($first_decode['dAccId'], $first_decode['dApikey'])) {
                ...
                if (is_wp_error($result)) {
                    ...
                    if (isset($result['subscribers'][0]['id'])) {
                        ...
                        $errors .= 'Failed to add subscriber. Check response details.'; 
                    }
                }
            } else {
                $errors .= 'Account ID, API key, email, and name are required fields.'; 
            }
        ...
        if (in_array('ConvertKit', $actionValues)) {
            ...
            if (is_wp_error($response)) {
                $error_message = $response->get_error_message();
                $errors = 'Failed to connect to ConvertKit API. Error: ' . $error_message;
            } else {
                ...
                if ($status_code === 200) {
                    ...
                    $response_data['data'] = 'Subscriber added successfully to ConvertKit.';
                    $actions_success['convertkit'] = true;
                } else {
                    $errors .= 'Failed to submit form to ConvertKit. Please check your input and try again.';
                }
            }
        ...
        if (in_array('GetResponse', $actionValues)) {
            ...
            if (is_wp_error($response)) {
                $error_message = $response->get_error_message();
                $errors = 'Failed to connect to GetResponse API. Error: ' . $error_message;
            } else {
                ...
                if ($response_code === 202) {
                    $response_data['success'] = true;
                    $response_data['data'] = 'Form submitted successfully to GetResponse.';
                    $actions_success['getresponse'] = true; 
                } else {
                    $errors = 'Failed to submit form to GetResponse. Please check your input and try again.';
                }
            }
        ...
        if (in_array('Mailchimp', $actionValues)) {
            ...
            if (strpos($mc_api_key, '-') === false) {
                $errors .= 'Invalid Mailchimp API key format.';
                return;
            }
        
            $data_center = substr($mc_api_key, strpos($mc_api_key, '-') + 1);
            if (empty($data_center)) {
                $errors .= 'Mailchimp API key is missing the data center.';
                return;
            }
            ...
            if (is_wp_error($response)) {
                $error_message = $response->get_error_message();
                $errors .= 'Failed to connect to Mailchimp API. Error: ' . $error_message;
            } else {
                $response_code = wp_remote_retrieve_response_code($response);
                $response_body = wp_remote_retrieve_body($response);
                $response_data = json_decode($response_body, true);
                ...
                if ($response_code >= 200 && $response_code < 300) {
                    $response_data['success'] = true;
                    $response_data['data'] = 'Subscriber added successfully to Mailchimp.';
                    $actions_success['mailchimp'] = true;
                } else {
                    $errors .= "Mailchimp API error: {$response_data['detail']}";
                }
            }
        ...
        if (in_array('Brevo', $actionValues)) {
			...
			if (is_wp_error($response)) {
				...
				if ($response_code !== 201) {
					...
					$response_data['data'] = 'Contact added successfully';
        ...
        if (isset($_POST['securityValue']) && $_POST['securityValue'] === "recaptcha") {
            if (isset($_POST['g-recaptcha-response']) && isset($_POST['captchaopt'])) {
                ...
                switch ($captchaopt) {
                    ...
                    case 'v3': 
                        ...
                        break;
                    default:
                        wp_send_json_error(['message' => 'Invalid captcha option.', 'captchaopt' => $captchaopt]);
                        return;
                }
                ...
                if (is_wp_error($recaptcha_response_message)) {
                    wp_send_json_error('Failed to validate reCAPTCHA. Error: ' . $recaptcha_response_message->get_error_message());
                    return;
                }
                ...
                if ($captchaopt === 'v3') {
                    if (isset($result['success']) && $result['success'] && isset($result['score']) && $result['score'] >= 0.5) {
                        $actions_success['captcha'] = true;
                    } else {
                        $errors .= 'reCAPTCHA v3 validation failed or the score is too low. '; 
                        $actions_success['captcha'] = false;
                    }
                } else {
                    $actions_success['captcha'] = (isset($result['success']) && $result['success']) ? true : ($errors .= 'reCAPTCHA validation failed. Please try again. ' && false);
    
                }
            } else {
                $errors .= 'No reCAPTCHA response or captcha option received. ';
                $actions_success['captcha'] = false;
            }
            ...
            if (isset($_POST['g-recaptcha-response'])) {
                ...
                if (is_wp_error($cloudflare_response_message)) {
                    ...
                    $errors .= 'Failed to validate Cloudflare Turnstile. Error: ' . $error_message;
                } else {
                    $cloudflare_body = wp_remote_retrieve_body($cloudflare_response_message);
                    $cloudflare_result = json_decode($cloudflare_body, true);
        
                    $response['data'] .= (isset($cloudflare_result['success']) && $cloudflare_result['success']) ? 'Cloudflare Turnstile validated successfully.' : ($errors .= 'Cloudflare Turnstile validation failed. Please check again.');
                    $actions_success['cloudflare'] = (isset($cloudflare_result['success']) && $cloudflare_result['success']);
    
                }
            } else {
                $errors .= 'No Cloudflare Turnstile response received.';
            }
        
            !empty($errors) ? wp_send_json_error($errors) : wp_send_json_success($response['data']);
