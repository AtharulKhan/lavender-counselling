	...
	public static function global_attr_magic_scroll($wrapAttr = '', $attr = []){
		if(!empty($attr['PlusMagicScroll']) && !empty($attr['MSFrame']) ){
			...
			if(isset($attr['MSView']) && !empty($attr['MSView']) ){
				...
				$wrapAttr .= ' data-tpgb-msview="'.htmlspecialchars(wp_json_encode($devices), ENT_QUOTES, 'UTF-8').'"';
			}
			if(!empty($scrollAttr)){
				$wrapAttr .= ' data-tpgb-ms="'.htmlspecialchars(wp_json_encode($scrollAttr), ENT_QUOTES, 'UTF-8').'"';
			}
		}
		return $wrapAttr;
	}
