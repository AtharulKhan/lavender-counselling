        ...
        tabsWrappers.forEach(function(currentTab) {
            ...
            if(hash){
                ...
                if(hashWrap && !hashWrap.classList.contains("active")){
                    ...
                    let getDataWrap = document.querySelector(".tpgb-tab-content[data-tab='" + tab_index + "']")
                    if(getDataWrap){
                        window.scrollTo({
...
function tabstoursclickhover(e){
    ...
    let getTabCwrap = tabsContainer.querySelector(":scope > .tpgb-tabs-content-wrapper");
    let activeContent = getTabCwrap.querySelector(":scope > .tpgb-tab-content[data-tab='" + currentTabIndex + "']");
    activeContent.classList.add('active');
    activeContent.classList.remove('inactive');
    
    let childTabsWrapper = activeContent.querySelector('.tpgb-tabs-wrapper');
    if (childTabsWrapper) {
        let firstChildTab = childTabsWrapper.querySelector('.tpgb-tab-li .tpgb-tab-header.active');
        if (firstChildTab) {
            firstChildTab.click();
        }
    }
