...
if ( ! class_exists( 'WPI_Location_Post_Format' ) ) {
	class WPI_Location_Post_Format extends WPI_Location {

		public function initialize() {
			$this->name = 'post_format';
			$this->label = __( 'Post format', 'wp-interactions' );
			$this->category = 'post';
		}
		...
		public function get_values() {
			...
			if ( ! $has_standard ) {
				array_unshift( $options, [
					'value' => 'standard',
					'label' => __( 'Standard', 'wp-interactions' ),
				] );
			}
	...
	wpi_add_location_rule_type( 'post_format', 'WPI_Location_Post_Format' );
}
