...
if ( ! class_exists( 'WPI_Action_Type_Background_Color' ) ) {
	class WPI_Action_Type_Background_Color extends WPI_Abstract_Action_Type {
		public function initialize() {
			...
			$this->label = __( 'Background Color', 'wp-interactions' );
			$this->description = __( 'Changes the background color', 'wp-interactions' );
			...
			$this->properties = [
				'color' => [
					'name' => __( 'Background Color', 'wp-interactions' ),
					'type' => 'color',
					'default' => '#000000',
				],
			];
	...
	wpi_add_action_type( 'backgroundColor', 'WPI_Action_Type_Background_Color' );
}
