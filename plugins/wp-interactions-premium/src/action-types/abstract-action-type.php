...
if ( ! class_exists( 'WPI_Abstract_Action_Type' ) ) {
	class WPI_Abstract_Action_Type {
		...
		public function get_frontend_inline_script() {
			$path = plugin_dir_path( WPINTERACTIONS_FILE ) . "dist/frontend/actions/{$this->name}.php";
			$path = apply_filters( "wpi/action/inline-path/{$this->name}", $path );

			// Load the rest api url and nonce JS variables in the frontend if needed.
			if ( $this->uses_frontend_rest_api ) {
				WPI_Frontend::enqueue_rest_script_params();
			}
