...
if ( ! class_exists( 'WPI_Interaction' ) ) {
	class WPI_Interaction {
		private $post = [];
		...
		public static function update( $interaction_data ) {
			$post_arr = [
				...
				'post_status' => $interaction_data['active'] ? 'publish' : 'wpi-inactive',
			];
			return $post_arr['ID'] === 0 ? wp_insert_post( $post_arr ) : wp_update_post( $post_arr );
		}
		...
		public static function secure_interaction_data( $interaction_data ) {
			foreach ( $interaction_data['timelines'] as $timeline_index => $timeline ) {
				foreach ( $timeline['actions'] as $action_index => $action ) {
					...
					$action_config = wpi_get_action_type( $action_type );
					$action_value = $action['value'];

					if ( $action_config->verify_integrity ) {
						$signature = hash_hmac( 'sha256', json_encode( $action_value ), wpi_salt() );
		...
		public static function trash( $interaction_key ) {
			return wp_trash_post( self::get_post_id_from_key( $interaction_key ) );
		}
		...
		public function should_render_in_frontend() {
			...
			$screen = WPI_Frontend_Screen::get_instance();
